<template>
    <div class="orderDetail fullscreen">
        ...
        <div class="list">
            ...
            >
                <template #footer>
                    ...
                    <van-icon
                        class="evaluate"
                        name="comment-o"
                        v-if="info.orderStatus == 2 || info.orderStatus == 8"
                        @click="
                            evaluate(
                                item.goodsId,
                                item.mainTitle,
                                item.selectedSkuText,
                                item.goodsSpecsId
                            )
                        "
                    />
            ...
            <div
                class="payTime"
                v-if="info.payTimeText"
                v-text="'支付时间：' + info.payTimeText"
            ></div>
            <div
                class="payTime"
                v-if="info.orderStatus == 4"
                v-text="'申请退款时间：' + info.updatedAtText"
            ></div>
            <div
                class="payTime"
                v-if="info.orderStatus == 5"
                v-text="'申请退货退款时间：' + info.updatedAtText"
            ></div>
            <div
                class="payTime"
                v-if="info.orderStatus == 3"
                v-text="'取消时间：' + info.updatedAtText"
            ></div>
        </div>
            ...
            post(api.getOrderDetail, params)
                .then((res) => {
                    ...
                    if (dataa.code == 0) {
                        ...
                        data.payTimeText =
                            data.payTime != null
                                ? formatDate(new Date(data.payTime))
                                : "";
                        data.updatedAtText =
                            data.updatedAt != null
