    ...
    <div class="goodsEvaluate">
        ...
        <div class="list">
            <div class="item" v-for="item in list" :key="item.id">
                ...
                <van-rate class="rate" v-model="item.rate / 2" :size="12" void-color="#888787" color="#3391c9"/>
                <div class="clearBoth"></div>
                <div class="comment" v-if="item.comment" v-text="item.comment"></div>
            </div>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            getGoodsEvaluateList() {
                ...
                post(api.getGoodsEvaluateList, params).then(res => {
                    let data = res.data;
                    if (data.code == 0) {
                        let list = [];
                        data.data.forEach(v => {
